...
// Creating montages of individual worms microscopy pictures. Each worm is cropped, aligned, and straightened to facilitate visual comparison.
// Hanneke Okkenhaug, Feb2020, Babraham Institute, Imaging Facility
// Script has been then modified and adapted for Wormscape.
// Adrien Assie, February 2024

macro "Worm-align"{

//If you ducked up the previous run
cont=getBoolean("Did you mess up the previous run?");

if(cont==1){
	waitForUser("Please select the folder that contains the image you already processed");
	ori = getDirectory("Please select the folder that contains the image you already processed");
	olist = getFileList(ori);
}
